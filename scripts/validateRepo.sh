...
if [ $# -lt 3 ]; then
    echo "Usage: $0 <Pixel Definitions Dir> <USER_MAP> <ASANA_PROJECT> [RELEASE_DEFINITIONS_DIR]"
...
if [ -n "$RELEASE_MAIN_DIR" ]; then
    ...
    echo "No release definitions provided; validating HEAD definitions only"
fi
